...
const topics = [
  ...
  },
  {
    ...
    preferredDomains: ["arxiv.org", "github.com", "medium.com", "towardsdatascience.com"],
    excludeDomains: [],
...
async function createTopics() {
  try {
    // Create agent without authentication (will use anonymous principal)
    const agent = new HttpAgent({
      host: 'https://ic0.app'
    });

    // Create actor
    const canisterId = 'wvset-niaaa-aaaao-a4osa-cai';
    ...
    for (const topicData of topics) {
      ...
      try {
        ...
        if (result && 'Ok' in result) {
          ...
          console.log(`❌ Unexpected response for ${topicData.name}`);
        }
      } catch (error) {
        console.log(`❌ Error creating ${topicData.name}:`, error.message);
        if (error.message.includes('Unauthorized')) {
          console.log('    Note: You need to be logged in as an admin to create topics');
        }
      }
    }
    ...
    console.log('\nIf you got "Unauthorized" errors, please:');
    console.log('1. Log into the admin dashboard first');
    console.log('2. Then manually add topics through the UI');
    console.log('3. Or update this script to use authenticated identity');
