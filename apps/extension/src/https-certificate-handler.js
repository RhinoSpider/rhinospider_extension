/**
 * https-certificate-handler.js - Handles HTTPS certificate verification for RhinoSpider extensions
 * 
 * This script handles certificate verification for the RhinoSpider extension when connecting to
 * the proxy servers via HTTPS. It adds a special handler for dealing with self-signed certificates.
 */
...
window.fetch = async function(resource, init) {
  try {
    ...
    if (isRhinoSpiderDomain && error.message.includes('certificate') || error.name === 'TypeError') {
      console.warn(`Certificate error detected for ${url}, retrying with HTTP fallback`);
...
XMLHttpRequest.prototype.open = function(method, url, ...args) {
  ...
  if (isRhinoSpiderDomain && url.startsWith('https://')) {
    const httpUrl = url.replace('https://', 'http://');
    console.warn(`Converting HTTPS to HTTP for ${url}`);
...
// Optional: Report this change to the console
