// Test the /api/submit endpoint with all required parameters
...
console.log(`${GREEN}=== Testing /api/submit Endpoint ===${RESET}`);
...
async function testSubmitEndpoint() {
  const url = `http://${SERVER_HOST}:${SERVER_PORT}/api/submit`;
    ...
    if (contentType && contentType.includes('application/json')) {
      try {
        ...
        if (response.status === 200) {
          console.log(`${GREEN}✓ Successfully submitted data to the /api/submit endpoint${RESET}`);
